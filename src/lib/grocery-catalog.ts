...
const ROWS: Row[] = [
  ...
  [
    ...
    "g",
    "Pantry",
    { Pantry: 12, Fridge: 21, Freezer: 180 },
    ["arbi", "arvi", "arabi", "taro", "taro root", "colocasia", "ghuiya", "kachalu"],
  ],
  [
    "Yam",
    "Vegetables",
    "g",
    "Pantry",
    { Pantry: 21, Fridge: 30, Freezer: 180 },
    ["jimikand", "suran", "elephant foot yam"],
  ],
  [
    "Raw Banana",
    "Vegetables",
    "pcs",
    "Fridge",
    { Fridge: 10, Pantry: 5, Freezer: 120 },
    ["kacha kela", "plantain"],
  ],
...
/**
 * Extra everyday / Hindi names so search finds a product however it is typed.
 * Keyed by product id (the slug of the catalog name).
 */
const EXTRA_ALIASES: Record<string, string[]> = {
  garlic: ["lehsun", "lahsun", "lasan", "lasun", "vellulli"],
  ginger: ["adrak", "aadrak", "inji"],
  onions: ["pyaz", "pyaaz", "kanda", "piyaz"],
  potatoes: ["aloo", "alu", "batata"],
  tomatoes: ["tamatar", "tamater"],
  spinach: ["palak", "paalak"],
  "green-chillies": ["hari mirch", "mirchi", "chilli", "chili"],
  coriander: ["dhania", "hara dhania", "cilantro"],
  "curry-leaves": ["kadi patta", "curry patta"],
  cauliflower: ["gobi", "phool gobi", "gobhi"],
  cabbage: ["patta gobi", "band gobi"],
  "lady-finger": ["bhindi", "okra", "bhendi"],
  brinjal: ["baingan", "bengan", "eggplant", "aubergine"],
  "bottle-gourd": ["lauki", "ghiya", "dudhi"],
  "bitter-gourd": ["karela", "kerala"],
  "ridge-gourd": ["turai", "tori"],
  carrots: ["gajar"],
  "green-peas": ["matar", "mutter"],
  cucumber: ["kheera", "khira"],
  radish: ["mooli"],
  beetroot: ["chukandar"],
  pumpkin: ["kaddu", "sitaphal kaddu"],
  capsicum: ["shimla mirch", "bell pepper"],
  mushrooms: ["khumb", "khumbi"],
  "sweet-potato": ["shakarkandi", "shakarkand"],
  "spring-onion": ["hara pyaz"],
  lemons: ["nimbu", "neembu", "lime"],
  curd: ["dahi", "yoghurt", "yogurt", "curd rice"],
  milk: ["doodh", "dudh"],
  butter: ["makhan"],
  ghee: ["clarified butter", "desi ghee"],
  paneer: ["cottage cheese", "panir"],
  eggs: ["anda", "ande"],
  chicken: ["murgi", "murga", "kukkad"],
  "chicken-breast": ["murgi breast", "boneless chicken"],
  ...
  "sweet-lime": ["mosambi", "musambi"],
  coconut: ["nariyal", "narial"],
  dates: ["khajoor", "khajur"],
  turmeric: ["haldi"],
  "red-chilli-powder": ["lal mirch", "mirchi powder"],
  ...
  "carom-seeds": ["ajwain"],
  "fennel-seeds": ["saunf"],
  "fenugreek-seeds": ["methi dana", "methi"],
...
export const GROCERY_CATALOG: GroceryProduct[] = ROWS.map(
  ([name, category, unit, storage, shelf, aliases], index) => ({
    ...
    shelf,
    rank: index,
    aliases: [...new Set([...(aliases ?? []), ...(EXTRA_ALIASES[slug(name)] ?? [])])],
  }),
);
